...
class Solution {
private:
    bool inPositions(int x, int y, int countX, int countY, multiset<vector<int>> positions) {
        int divideX = x / countX;
        int divideY = y / countY;
        if (divideX == divideY) {
            if (positions.find({x % countX, y % countY}) != positions.end()) {
                return true;
            }
        } else if (x % countX == 0 && divideY + 1 == divideX) {
            if (positions.find({x % countX + countX, y % countY}) != positions.end()) {
                return true;
            }
        } else if (y % countY == 0 && divideX + 1 == divideY) {
            if (positions.find({x % countX, y % countY + countY}) != positions.end()) {
                return true;
            }
        }
        return false;
...
public:
    bool robot(string command, vector<vector<int>> &obstacles, int x, int y) {
        int curX = 0, curY = 0, countX = (int) count(command.begin(), command.end(), 'R'), countY = (int) count(
                command.begin(),
                command.end(),
                'U');
        multiset<vector<int>> obstaclesSet(obstacles.begin(), obstacles.end());
        multiset<vector<int>> positions;
        for (const char &cmd: command) {
            positions.insert({curX, curY});
            if (cmd == 'R') {
                curX++;
            } else {
                curY++;
        ...
        for (const vector<int> &obstacle: obstaclesSet) {
            if (obstacle[0] > x) {
                break;
            }
            if (obstacle[1] > y) {
            ...
            if (inPositions(obstacle[0], obstacle[1], countX, countY, positions)) {
                return false;
            }
        }
        return inPositions(x, y, countX, countY, positions);
    }
};
...
int main() {
    ...
    //
    vector<vector<int>> obstacles3 = {{7, 7},
                                      {0, 5},
                                      {2, 7},
                                      {8, 6},
                                      {8, 7},
                                      ...
                                      {0, 3},
                                      {3, 6}};
    vector<vector<int>> obstacles4 = {{3, 0}};
    vector<vector<int>> obstacles2 = {{35340729, 26552041},
                                      {41315902, 31041191},
                                      {35435801, 26623424},
                                      {35985598, 27036459},
                                      {35343672, 26554242},
...
//            obstacles1,
//            35339967, 26551450);
//    cout << solution.robot("URR", obstacles1, 3, 2);
//    cout << solution.robot("URRURRR", obstacles3, 4915, 1966);
    cout << solution.robot("RRRUUU", obstacles4, 3, 3);
    return 0;
}